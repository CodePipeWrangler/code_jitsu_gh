...
def print_usage():
    ...
    Usage: python script_name.py -i INPUT_FILE -r REPEAT_CLASSES [-o OUTPUT_FILE] [-l CHROM_LENGTHS] [-w WINDOW_SIZE]
                                 [--use_ranges] [-f FILTER_FILE] [-d DELIMITER] [-c COLOR_BREAK]
    ...
    Arguments:
      ...
      -l, --chrom_lengths  Optional input file with chromosome lengths
      -w, --window_size    Window size for density calculation (optional)
      --use_ranges         Flag to indicate that repeat classes should be treated as ranges (optional)
      -f, --filter_file    Path to the JSON file containing filter criteria (optional)
      -d, --delimiter      Delimiter used in the input file (optional, default is tab)
      -c, --color_break    Integer describing how far apart "s. start" values must be to be grouped into a
                           different category. Default is 100 Kb. (optional)
...
def parse_args():
    ...
    parser.add_argument('--use_ranges', action='store_true', help="Set this flag to indicate repeat classes should be treated as ranges.")
...
def main():
   ...
   #     filtered_df = df
    ...
    if args.chrom_lengths:
        ...
        max_chrom_length = original_max_chrom_length_logic()
...
#### I grabbed this code below from my visualization program for BLAST. Check to see this code is not majorly altered from the original version. With a couple tweaks this code should be ready to run
